...
import (
	...
	"github.com/zalgonoise/x/politebot"
	"github.com/zalgonoise/x/politebot/internal/log"
	"github.com/zalgonoise/x/politebot/internal/repository/memory"
	"github.com/zalgonoise/x/politebot/internal/repository/sqlite"
)
...
type Command interface {
	...
	Elements() []politebot.ApplicationCommandOpts
}
...
func ExecBot(ctx context.Context, logger *slog.Logger, args []string) (int, error) {
	...
	adminUsers := fs.String("adminlist", "", "comma-separated list of admin user IDs that should be able to add or remove any number of angy points")
	serverID := fs.String("server", "", "discord server ID")
	appID := fs.String("app", "", "discord app ID")
	role := fs.String("role", "", "discord role ID to allow users to give angy points")
	nonAdminMaxAngyPoints := fs.Int("max-cookies", 1, "maximum number of angy points regular users can give")
	thresh := fs.Duration("thresh", time.Hour*24, "duration between adding angy points")
	...
	for _, command := range commands {
		handler, cmd := politebot.RegisterSlashCommand(logger, command.Name(), command.Callback, command.Elements()...)
	...
	logger.InfoContext(ctx, "connected to discord",
		...
		slog.Int("non_admin_max_angy_points", *nonAdminMaxAngyPoints),
	)
