...
func (g *mapGraph[T, I, V]) Add(nodes ...model.Node[T, I, V]) error {
	return AddNodesToMap[T, I, V](g, nodes...)
}
func (g *mapGraph[T, I, V]) Remove(nodes ...T) error {
	if g.conf.Immutable {
		return errs.Immutable
	}
	return RemoveNodesFromMap[T, I, V](g, nodes...)
}
func (g *mapGraph[T, I, V]) Get(node T) (model.Node[T, I, V], error) {
...
func (g *mapGraph[T, I, V]) Disconnect(from, to T) error {
	if g.conf.Immutable {
		return errs.Immutable
	}
	return AddEdgeInMap[T, I, V](g, from, to, 0, g.conf.IsNonDirectional, g.conf.IsNonCyclical)
}
func (g *mapGraph[T, I, V]) Edges(node T) ([]model.Node[T, I, V], error) {
	return GetEdgesFromMapNode[T, I, V](g, node)
}
func (g *mapGraph[T, I, V]) Weight(from, to T) (I, error) {
