...
//
// Exporter will periodically receive data either from the Processor or its Aggregator functions. The data
...
type Exporter interface {
	...
	//
	// The Exporter may have a set of Aggregator configured -- in that case it send the audio data it receives to all
	// Aggregator. It is also the responsibility of the implementation of Exporter to properly send those aggregations
	...
	// target, or from any errors raised by the configured Aggregator types.
	Export(data []float64) error
