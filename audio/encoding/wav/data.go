...
func NewChunk(h *data.Header, bitDepth, format uint16) Chunk {
	if h != nil && string(h.Subchunk2ID[:]) == data.JunkIDString {
		return data.NewJunk(h)
	}

	switch format {
	case UnsetFormat, PCMFormat:
		switch bitDepth {
		case 0:
			return data.NewJunk(h)
		case bitDepth8, bitDepth16, bitDepth24, bitDepth32:
			return data.NewPCMChunk(bitDepth, h)
		default:
			return nil
		}
	case FloatFormat:
		return data.NewFloatChunk(bitDepth, h)
	default:
		return nil
	}
}
...
func NewRingChunk(h *data.Header, bitDepth, format uint16, size int, proc func([]float64) error) Chunk {
	...
	case UnsetFormat, PCMFormat:
		...
		case 0:
			return data.NewJunk(h)
		case bitDepth8, bitDepth16, bitDepth24, bitDepth32:
			return data.NewPCMRing(bitDepth, h, size, proc)
		default:
			return nil
		}
	case FloatFormat:
		return data.NewFloatRing(bitDepth, h, size, proc)
	default:
		return nil
	}
}
