...
func TestNewWav(t *testing.T) {
	...
	if !bytes.Equal(wants, w.Header.Bytes()) {
...
func TestWavWriteRead(t *testing.T) {
	...
	for idx, test := range testdata {
		...
		if _, err = newWav.Write(buf); err != nil {
			t.Errorf("2nd-pass decoding error on index %d: %v", idx, err)
			continue
		}
...
//nolint:gocognit // this test function contains 4 subtests, but they are not actually as complex as this warning shows
	...
	for _, test := range testdata[:4] {
		...
		b.Run("All", func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				// Process
				w.Data.Apply(filters.PhaseFlip(), filters.PhaseFlip())
